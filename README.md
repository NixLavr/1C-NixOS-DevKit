...
пути Apache (`/usr/sbin/httpd` и `/etc/httpd/conf/httpd.conf`). Последний
ведёт на изменяемый `/var/lib/1c-web/httpd.conf`, а Apache подключает именно
этот совместимый путь. Поэтому публикация работает как при вызове `webinst`
из `/opt/1cv8/x86_64/<версия>`, так и при прямой записи Конфигуратора. Файл
сохраняется после `nixos-rebuild`, а Apache автоматически проверяется и
перезагружается после изменения.
