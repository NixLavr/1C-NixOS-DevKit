  ...
  stdenv.mkDerivation {
    ...
    installPhase = ''
      ...
      ${lib.optionalString isClient ''
        ...
        # Ровно та же развилка 4.0/4.1, что и у libwx выше, но для
        # веб-расширения WebKit — про него легко забыть, потому что оно
        # никем не NEEDED: его грузит сам WebKitWebProcess по имени файла.
        # Поставка несёт webkit2_extu-3.0.so (ветка webkit2gtk 4.0 /
        # libsoup 2.4, вырезанная из nixpkgs как EOL — libwebkit2gtk-4.0.so.37
        # нет вообще нигде в store) и webkit2_extu-3.0.so.wk41 под
        # webkit 4.1, которую мы реально ставим. Если расширение не
        # загрузилось, HTML-виджеты управляемых форм (планировщик,
        # календарь, диаграмма Ганта и т.п.) остаются ПУСТЫМИ, при этом ни
        # ошибки, ни падения не будет — сама форма работает, поэтому
        # симптом выглядит как "не хватает какой-то зависимости".
        # Кладём на место дефолтного имени .wk41-вариант.
        ...
        void *dlopen(const char *file, int flags) {
          ...
          if (file) {
            /* grphcs.so догружает через dlopen СВОЙ бандловый
               ...
               голый libcairo.so.2 нельзя — Fc*, FT_* и hb_* тогда
